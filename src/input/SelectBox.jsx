  ...
  return (
    <Listbox ref={ref} value={value} onChange={onChange}>
      <div className="relative mt-1">
        <Listbox.Button
          className={`relative w-full cursor-default rounded-md bg-white border pr-10 text-left focus:outline-none focus:ring-1 ${sizes[size]} ${hasError
            ? 'border-red-300 focus:ring-red-400 focus:border-red-400'
            : 'border-gray-300 focus:border-green-120 focus:ring-green-120'
          }`}
        >
          <span
            className={`${selected ? '' : hasError ? 'text-red-500' : 'text-gray-400'} block truncate`}
          >
