...
const TeacherReg = (props) => {
  ...
  const formSchema = z
  .object({
    username: z.string().min(1, "Username is required").max(50).nonempty().regex(/^(?!.*\.com).*$/i, "Username cannot contain '.com'"),
...
<button   onClick={() => props.onFormSwitch('Login')}>Already Register?<strong> Login</strong></button>
</div>
        </div>
      </div>
    </section>
